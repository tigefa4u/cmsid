...
function classic_theme_option( $param = null, $args = '' ){	
	$defaults = array(
		'limit_post' => 10
	);
	
	if( checked_option( 'classic_options' ) && empty($args) ){
		$args = get_option('classic_options');
...
<style type="text/css">
#main{width:98%; float:left; margin-left:10px;}
</style>
<?php	
}elseif( 
...
<style type="text/css">
#main{width:76%;float:left; margin-left:10px;}
...
if( $data->view == 'page-full' ){?>
    <?php if ( !empty($data->title) ) : ?>
		<div class="bg" style="margin-top:5px;"><?php echo $data->browse?></div>
...
<?php }elseif( $data->view == 'archive' ){?>
    ...
    <span class="waktu_posting"><?php echo datetimes2( $content[date] ); ?></span>	
    </p>
  <?php endforeach; ?>
  	<div class="clear"></div>
