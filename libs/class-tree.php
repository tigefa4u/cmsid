...
class Tree {
	 ...
	 * @return string
	 */
	function generate_list($ul_attr = '', $ul_visible = true) {
		return $this->ul(0, $ul_attr, $ul_visible);
	}
	 ...
	 * @return string
	 */
	function ul($parent = 0, $attr = '', $ul_visible = true ) {
		static $i = 1;
		$indent = str_repeat("\t\t", $i);
		if (isset($this->data[$parent])) {
			...
			foreach ($this->data[$parent] as $row) {
				...
				$att_padding_left = '';
				if( is_admin() )
					$att_padding_left = 'style="padding-left: 30px; padding-right: 0px;"';
					
				$child = $this->ul($row['id'], $att_padding_left);
				$html .= "\n\t$indent";
				$html .= '<li'. $row['li_attr'] . '>';
				$html .= $row['label'];
				if ($child) {
